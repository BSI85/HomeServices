(() => {
    ...
    async function loadBlogItems() {
        ...
        if (response.ok) {
            ...
            data = responseResult;
            initBlog(data, start, end);
        } else alert("ALARM!!!");
    }
    function initBlog(data, start, end) {
        let dataSliced = data.items.slice(start, end);
    ...
    function viewMore() {
        if (data.items.length == document.querySelectorAll(".item-blog").length) document.querySelector(".blog__view-more").classList.add("hidden");
    }
    document.addEventListener("click", (function(e) {
        const targetElement = e.target;
        if (targetElement.closest(".blog__view-more")) {
            start = document.querySelectorAll(".item-blog").length;
            end = start + 3;
            initBlog(data, start, end);
            viewMore();
            e.preventDefault();
